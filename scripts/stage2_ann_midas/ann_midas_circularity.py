"""
Stage 2 (Robustness): ANN-MIDAS Circularity Assessment
Evaluates potential circularity in temporal disaggregation by testing
alternative indicator subsets.
"""
...
def prepare_tensors_with_ar_and_shocks(df_annual, df_quarterly, df_monthly, max_m_lag=12, max_q_lag=4):
    ...
    m_cols = ['US_IndustrialProduction', 'GSCPI']
    q_cols = ['ValueAdded']
...
def compute_linear_baseline(df_annual, df_quarterly, df_monthly):
    ...
    for y in years:
        ...
        m_exp = df_m[df_m['Year'] == y]['US_IndustrialProduction'].sum()
    ...
    for y in years:
        ...
        m_v = df_m[df_m['Year'] == y]['US_IndustrialProduction'].values
...
def execute_fully_constrained_disaggregation(df_annual, df_quarterly, df_monthly):
    ...
    param_grid = {'hidden_nodes': [3, 5, 8], 'lr': [0.01, 0.005], 'epochs': [500,800], 'q_lags': [[4]], 'm_lags': [[12,12], [6,12]]}
    ...
    with torch.no_grad():
        ...
        # Apply Clamping
        expanded = raw_q * np.repeat(ind / np.mean(ind), 4)
        expanded[:4] = raw_q[:4]
        expanded[-12:] = raw_q[-12:]
        
        final_ann = dentonm(expanded, Y_t.numpy().flatten(), freq="aq")
...
# final_data.to_csv("Interpolated_TiVA_DVA_Circularity.csv", index=False)
...
final_data.to_csv("Interpolated_TiVA_DVA_Circularity.csv", index=False)
print("Saved Interpolated_TiVA_DVA_Circularity.csv")